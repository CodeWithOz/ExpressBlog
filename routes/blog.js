...
const { orgUrl, token } = require('../credentials');
...
router.get('/', (req, res) => {
  models.Post.findAll({ order: sequelize.literal('createdAt DESC') }).then(
    posts => {
      let postData = [];
...
router.post('/:slug/delete', (req, res, next) => {
  // TODO: delete a post while logged out
  // because ensureAuthenticated is not used here
  models.Post.findOne({
    where: {
      slug: req.params.slug,
      authorId: req.user.id
